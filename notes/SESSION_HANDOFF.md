...
- **STILL NOT DONE (these PLACE BROKER ORDERS — do deliberately under paper, read-only OFF):** run_stack_handler (instrument→contract→broker orders, execution). Also: mandatory position limits + shadow_cost for real runs; re-estimate as data matures; decide CAD base (needs CAD FX).
