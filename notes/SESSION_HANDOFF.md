...
- **NEXT (production, not yet done):** set mandatory position limits (interactive_controls; required for dynamic opt) + shadow_cost in private_config; then run_strategy_order_generator (raw→integer optimal positions→instrument orders) and run_stack_handler (execution) — those PLACE ORDERS, so do under paper + read-only-off deliberately. Also: re-estimate/refresh as build-out grows; decide CAD base (needs CAD FX).
