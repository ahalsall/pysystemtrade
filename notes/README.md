...
- concepts/ — conceptual knowledge base:
  ...
  - [concepts/roll_cycles_and_liquid_contracts.md](concepts/roll_cycles_and_liquid_contracts.md) — why we hold liquid (often forward/seasonal) months not front; HoldRollCycle vs PricedRollCycle, CarryOffset, roll timing; validating rolls; the sparse-data too-far-forward trap; first-notice/delivery risk.
  ...
  - [concepts/csi_data.md](concepts/csi_data.md) — CSI Data (Unfair Advantage) as the deep-history source: decades of per-contract data, CSV format ~= our barchart pipeline, Windows-only (VM + auto-export), ~$540/yr. The deep-history complement to IB.
  - [concepts/csi_setup_guide.md](concepts/csi_setup_guide.md) — CSI Windows-VM + shared-folder + Unfair Advantage export setup, and the csi_pipeline ingester (rename + CSI config + reuse of the barchart pipeline).
  - [concepts/csi_export_design.md](concepts/csi_export_design.md) — OPTIMAL UA export settings for PST: raw individual contracts, max history, all UA continuation/adjust features OFF (PST does its own rolls+Panama); decision table + judgment calls.
  ...
  - [concepts/costs_and_capital.md](concepts/costs_and_capital.md) — minimum capital per contract (why micros matter), the SR-cost screen (24 of our 224 too expensive), and how dynamic opt helps.
  - [concepts/instrument_universe.md](concepts/instrument_universe.md) — the TWO universes: broad research/backtest set (501 = Rob ∩ rollconfig ∩ IB; CSI download target) vs curated live tradeable portfolio (~30 at CAD 150k; IB-Canada + cost + capital screened, micros for efficiency); screen on OUR data not Rob's.: IB Gateway/ib_async, production processes, order stacks, dynamic-opt in production, go-live checklist.
